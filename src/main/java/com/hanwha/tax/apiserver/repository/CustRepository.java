...
//    @Query(value="select a.cust_id from cust a order by a.cust_id desc limit 1" , nativeQuery=true)
    @Query(value="select a.cust_id from cust a where strcmp(DATE_FORMAT(CURRENT_DATE, '%y%m'), left(a.cust_id,4)) = 0 order by a.cust_id desc limit 1" , nativeQuery=true)
